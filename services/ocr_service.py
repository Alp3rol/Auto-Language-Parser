...
class OCRService:
    ...
    def extract_structured_blocks(self, pil_image: Image.Image) -> list:
        """
        Görseldeki metinleri blok ve satır bazında ayrıştırır.
        Her bir bloğun koordinatını (bbox), satır yüksekliğini, metnini ve başlık (heading) durumunu döndürür.
        """
        if pil_image is None:
            return []

        w_orig, h_orig = pil_image.size
        scale = 2.5 if h_orig < 120 else 1.8 if h_orig < 250 else 1.2
        ...
        raw_result = None
        if self.use_paddle_native:
            res = self.ocr_engine.ocr(img_np, cls=True)
            if res and res[0]:
                raw_result = res[0]
        else:
            res, _ = self.ocr_engine(img_np)
            if res:
                raw_result = res
        ...
        for res in raw_result:
            ...
            if text and score > 0.25:
                ...
                x1 = int(box[0][0] / scale)
                y1 = int(box[0][1] / scale)
                x2 = int(box[2][0] / scale)
                y2 = int(box[2][1] / scale)
                box_w = max(10, x2 - x1)
                box_h = max(10, y2 - y1)
                items.append({
                    "box": box,
        ...
        # Üst Y koordinatına göre sırala ve satır grupları oluştur
        ...
        for item in items_sorted:
            if current_y is None or abs(item["y"] - current_y) < max(10, item["h"] * 0.6):
                current_line.append(item)
                if current_y is None:
                    current_y = item["y"]
            else:
        ...
        all_heights = [item["h"] for item in items]
        avg_height = sum(all_heights) / len(all_heights) if all_heights else 16

        structured_blocks = []
        for line in lines:
            min_x = min(item["x"] for item in line)
            min_y = min(item["y"] for item in line)
            max_r = max(item["x"] + item["w"] for item in line)
            max_b = max(item["y"] + item["h"] for item in line)

            line_text = " ".join(item["text"] for item in line).strip()
            line_w = max_r - min_x
            line_h = max_b - min_y

            is_heading = line_h >= (avg_height * 1.3)
            is_bold = is_heading or (line_text.isupper() and len(line_text) < 30)
